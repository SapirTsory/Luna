...
import { ChatMessage, dateChipClass, ReadTicks, Reveal } from './ChatMessage'
...
export function ExternalSnippet({ icon, source, sender, delay, children }: ExternalSnippetProps) {
  return (
    <Reveal delay={delay} className="rounded-xl bg-whatsapp-bg p-2.5 opacity-90 font-whatsapp">
...
export function LunaBubble({ time, delay, children }: { time?: string; delay: number; children: ReactNode }) {
  return (
    <ChatMessage
      incoming
      delay={delay}
...
export function TimeDivider({ delay, children }: { delay: number; children: ReactNode }) {
  return (
    <Reveal delay={delay} className={dateChipClass}>
      {children}
    </Reveal>
  )
}
...
export function VoiceMessageBubble({ duration, time, delay }: { duration: string; time: string; delay: number }) {
  return (
    <ChatMessage
      delay={delay}
      className="flex flex-col items-end gap-1.5 self-end rounded-2xl rounded-tl-[3px] bg-whatsapp-bubble px-3 py-2.5 max-w-[85%]"
    >
